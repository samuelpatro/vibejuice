...
# Ad hoc signature. The cask strips the quarantine flag in a preflight step (same approach as
# many unsigned menu bar apps), so brew installs open without a Gatekeeper dialog. Direct DMG
# downloads still need `xattr -dr com.apple.quarantine` until the app is notarized.
...
TAPDIR="$(mktemp -d)"
if [[ -n "${TAP_TOKEN:-}" ]]; then
  git clone -q "https://x-access-token:${TAP_TOKEN}@github.com/$TAP_REPO.git" "$TAPDIR" 2>/dev/null || { echo "tap: clone failed"; exit 0; }
else
  gh repo clone "$TAP_REPO" "$TAPDIR" -- -q 2>/dev/null || { echo "tap: clone failed"; exit 0; }
fi
...
if git -C "$TAPDIR" diff --cached --quiet; then
  ...
  echo "tap updated: brew install --cask samuelpatro/tap/vibejuice"
else
