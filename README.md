...
- Tokens are refreshed by the CLIs, never by VibeJuice. When a Claude account's token has expired, VibeJuice runs one minimal headless Claude Code request as that account in a throwaway config directory, so Claude Code renews the token itself, then reads the result back into the vault and, for the active account, into Claude Code's own login slot. If Claude Code had already cleared its login after a failed refresh of its own, the row says so and offers Sign in. This costs one tiny Haiku message from that account. Right-click a row for "Refresh token" to trigger it by hand. Codex and Grok tokens still refresh only when their CLI runs as that account.
